module.exports = {
		...
		try{
			if(game_mode){
				if(data.text.match(/.google/)){
					var item = bot.system.splice(data, "google");
					if(item !== undefined){
						var search = item.replace(/ /g,'+');
						var ggl = 'http://lmgtfy.com/?q=' + search;
						bot.talk(ggl);
					}else if(err >= 1){
						bot.talk('i\'m sorry @' + data.name + ' i cannot let you do that');
						err = 0;
					}
				}
			}else{
    ...
    order: function(data){
		try{
			if(game_mode){
			    var item = bot.system.splice(data, "order");
				if(item !== undefined){
	...
	define: function(data){
		try{
			if(game_mode){
				var item = bot.system.splice(data, "define");
				var def = 'http://api.urbandictionary.com/v0/define?term=' + item;
				bot.$.ajax({
					url: def,
					dataType: "json",
					type: "GET",
